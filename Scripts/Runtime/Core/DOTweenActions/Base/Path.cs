...
namespace BrunoMikoski.AnimationSequencer
{
    ...
    public abstract class Path : SequencerAnimationBase
    {
        ...
        protected override Tweener GenerateTween_Internal(GameObject target, float duration)
        {
            ...
            if (!isLocal)
            {
                tween = target.transform.DOPath(GetPathPositions(), duration, pathType, pathMode, resolution,
                    gizmoColor);
                previousPosition = target.transform.position;
            }
            else
            {
                tween = target.transform.DOLocalPath(GetPathPositions(), duration, pathType, pathMode, resolution,
                    gizmoColor);
